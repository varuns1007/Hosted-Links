...
for (let i = 0; i < currentPage.length; i++){
  if (currentPage[i] === 'products') {
    ...
    if (currentPage.join('/') === `https://${shopDomain}.myshopify.com/cart?subtotalSelector`) {
    console.log('subtotalSelector');

      subtotalSelector();
    } else if (
      currentPage.join("/") ===
      `https://${shopDomain}.myshopify.com/cart?totalSelector`
    ) {
      console.log("totalSelector");
      totalSelector();
    } else if (
      currentPage.join("/") ===
      `https://${shopDomain}.myshopify.com/cart?individualPriceSelector`
    ) {
