...
const hostedUrl = "https://57b3699c2d67.ngrok.io";
...
function start() {
  ...
  function checkOut() {
    ...
    // if (user.cartSubTotalLocation.tagName.length > 0) {
    ...
    // if (user.individualProductLocation.tagName.length > 0) {
    //   if ((user.individualProductLocation.attribute.length = 0)) {
    //     products_Individual_Price_Array = document.querySelector(
    ...
    function setEventListeners(totalPriceTag) {
      ...
      let individualPriceDivs = [];
      let totalPriceDivs = [];
      for (var i = 0; i < products_Individual_Price_Array.length; i++) {
        if ((i + 2) % 2 === 0) {
          individualPriceDivs.push(products_Individual_Price_Array[i]);
        } else {
          totalPriceDivs.push(products_Individual_Price_Array[i]);
        }
      }
      ...
      for (let i = 0; i < individualProductDiscount.length; i++){
        if (individualProductDiscount[i] !== parseInt(products[i].price)) {
          products[i].priceDiv.innerHTML = `<del>Rs. ${
            products[i].price
          }</del> <ins style="text-decoration:none">Rs. ${String(
            String(individualProductDiscount[i])
          )}.00</ins>`;
        }
      }

      for (let i = 0; i < productTotalDiscount.length; i++){
        let originalTotal = parseInt(products[i].originalProductQuantity) * parseInt(products[i].price);
        if (productTotalDiscount[i] !== originalTotal) {
          products[i].totalPriceDiv.innerHTML = `<del>Rs. ${String(
            originalTotal
          )}</del> <ins style="text-decoration:none">Rs. ${String(
            String(productTotalDiscount[i])
          )}.00</ins>`;
        }
      }
        //individualPrice
        ...
        function inputEvent(e) {
          ...
          for (let i = 0; i < products.length; i++) {
            ...
            products[i].priceDiv = individualPriceDivs[i];
            products[i].totalPriceDiv = totalPriceDivs[i];
          }
          // console.log(products);
          // let subTotal = 0;
          // let discountTotal = 0;
