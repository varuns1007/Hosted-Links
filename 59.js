...
let hostedUrl = "https://d2b3e79975b2.ngrok.io"; //backend url
let frontendUrl = "https://d2b3e79975b2.ngrok.io"; //frontend url
...
function start() {
  ...
  function selectWidgetLocation() {
    ...
    p.innerText = "Select location for Tiered pricing while holding the Shift key.";
  ...
  function checkOut() {
    ...
    function setEventListeners(totalPriceTag) {
      ...
      if (user.Customtemplate.showCustomTemplate) {
        if
          (user.Customtemplate.location.subtotalPrice)
        // (true)
        {
          ...
          if (discountedSubtotal !== subTotal) {
            subTotal_Location.innerHTML = `<del style="font-size:0.8em;font-weight:600;color:grey">Rs. ${String(
              subTotal
            )}</del><br> <ins style="text-decoration:none">Rs. ${String(
      ...
      if (user.Customtemplate.showCustomTemplate) {
        if
          (user.Customtemplate.location.individualPrice)
          // (true)
        {
          for (let i = 0; i < individualProductDiscount.length; i++) {
            ...
            if (individualProductDiscount[i] !== parseInt(products[i].price)) {
              products[
                i
      ...
      if (user.Customtemplate.showCustomTemplate) {
        if (user.Customtemplate.location.totalPrice) {
          for (let i = 0; i < productTotalDiscount.length; i++) {
            ...
            let savedPrice =
              String(originalTotal - Math.round(productTotalDiscount[i]));
            let startIndex = customTemplate.search(":diff");
            ...
            if (productTotalDiscount[i] !== originalTotal) {
              products[i].totalPriceDiv.innerHTML = `<del style="font-size:0.8em;font-weight:600;color:grey">Rs. ${String(
                originalTotal
              )}</del><br> <ins style="text-decoration:none">Rs. ${String(
  ...
  function subtotalSelector() {
    ...
    let p = document.createElement("p");
    p.innerText =
      "Select cart subtotal amount while holding the Shift key.";
    let modalImage = document.createElement("img");
    modalImage.src =
      "https://cdn.jsdelivr.net/gh/varuns1007/Hosted-Links/subtotalImg.png";
    modalImage.style.width = "29em";
  ...
  function individualPriceSelector() {
    ...
    modalDiv.style.borderRadius = "4px";
    let p = document.createElement("p");
    p.innerText = "Select individual product price while holding the Shift key.";
    let modalImage = document.createElement("img");
    modalImage.src =
  ...
  function renderWidget() {
    ...
    if (user.tierDescription.templateType === 'custom') {
      ...
      function createP1Tag(quantity, price, discount) {
        ...
        let p;
        if
          (user.tierDescription.templateType === 'BuyQforE')
          // (1)
        {
          p = `<div style="padding-bottom: 1em">
                    <p style="color: ${
                      ...
                      Buy ${String(
                        ...
                        ><del>₹${String(price)}</del></span
                      >₹${String(price - discount)} each <span style="
                        color: ${
          ...
          (user.tierDescription.templateType === "BuyQforT")
          // (true)
        {
          p = `<div style="padding-bottom: 1em">
                    <p style="color: ${
                      ...
                      Buy ${String(
                        ...
                        ><del>₹${String(quantity * price)}</del></span
                      >₹${String(discountedTotal)} <span style="
                        color: ${
                          user.discountStyle[0].PricetextColor2 || "#f17157"
                        };
                        font-size: medium;
