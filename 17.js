...
for (let i = 0; i < currentPage.length; i++){
  ...
  } else if (currentPage[i] === 'cart' || currentPage[i] === 'cart?subtotalSelector' || currentPage[i] === 'cart?totalSelector' || currentPage[i] === 'cart?individualPriceSelector') {
...
function selectWidgetLocation() {
  ...
  function position(pos, node) {
    ...
    // console.log(obj);
    axios
      .post("http://localhost:5000/widgetSave",data)
      .then((result) => {
        console.log(result);
...
function checkOut() {
  ...
  checkoutButton[0].onclick = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:5000/custom-checkout", {
        // or 'PUT'
        products,
        shopDomain,
      })
