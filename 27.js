...
let axios2Tag = document.createElement("script");
axios2Tag.src = "https://cdn.jsdelivr.net/npm/axios/dist/axios.min.js";
document.head.appendChild(axios2Tag);
...
function selectWidgetLocation() {
  ...
  function position(pos, node) {
    ...
    // console.log(obj);
    axios
      .post("https://cefebd746185.ngrok.io/widgetSave", {
...
function checkOut() {
  ...
  checkoutButton[0].onclick = (e) => {
    e.preventDefault();
    axios
      .post("https://cefebd746185.ngrok.io/custom-checkout", {
        // or 'PUT'
        products,
        shopDomain,
      })
    ...
    if (event.shiftKey) {
      ...
      if (window.confirm("Confirm Selection")) {
        ...
        if (event.target.classList[0] === "clicked") {
          ...
          axios
            .post("https://cefebd746185.ngrok.io/subtotalSave", {
              className: className,
  ...
  function shiftClick(event) {
    if (event.shiftKey) {
      ...
      if (window.confirm("Confirm Selection")) {
        ...
        axios
          .post("https://cefebd746185.ngrok.io/totalSave", {
            className: className,
            tagName: event.target.tagName.toLowerCase(),
            shopName: shopDomain,
          })
  ...
  function shiftClick(event) {
    if (event.shiftKey) {
      ...
      if (window.confirm("Confirm Selection")) {
        ...
        axios
          .post("https://cefebd746185.ngrok.io/individualSave", {
            className: className,
            tagName: event.target.tagName.toLowerCase(),
            shopName: shopDomain,
          })
...
function renderWidget() {
  axios
    .post("https://cefebd746185.ngrok.io/userData", {
      username: shopDomain,
    })
    .then((response) => {
      console.log(response.data.data[0]);
      let node = document.querySelector(
        `${response.data.data[0].widgetLocation.div.tagName}.${response.data.data[0].widgetLocation.div.className}`
      );
      console.log(node);
      if (response.data.data[0].widgetLocation.location === "Below") {
        node.insertAdjacentHTML(
          "afterend",
          `<div
                    style="
      ...
      } else if (response.data.data[0].widgetLocation.location === "Above") {
        node.insertAdjacentHTML(
          "beforebegin",
          `<div
                    style="
